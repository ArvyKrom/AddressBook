...
int AddAddress(char *arg[], struct Person *list);
int AddAddressAt(char *arg[], struct Person *list);
int ClearBook(struct Person **list);
...
int Display(struct Person *list)
{
	unhide_every_address(list);
	if (display_addresses(list) == 1)
		printf("\033[1m(!)\033[0m There are no adddresses in the book\n");
	return 0;
}
...
int AddAddress(char *arg[], struct Person *list)
{
	...
	if (is_valid_address(arg[0], arg[1], arg[2], arg[3]) == VALID) {
		...
		printf("Successfully added a new address to the book\n");
		return 0;
	} else if (is_valid_address(arg[0], arg[1], arg[2], arg[3]) == WRONG_NAME_LENGTH) {
		printf("\033[1m(!)\033[0m Looks like the name that you've entered was too long\n");
		return 1;
	} else if (is_valid_address(arg[0], arg[1], arg[2], arg[3]) == WRONG_SURNAME_LENGTH) {
		printf("\033[1m(!)\033[0m Looks like the surname that you've entered was too long\n");
		return 1;
	} else if (is_valid_address(arg[0], arg[1], arg[2], arg[3]) == WRONG_EMAIL_LENGTH) {
		printf("\033[1m(!)\033[0m Looks like the email that you've entered was too long\n");
		return 1;
	} else if (is_valid_address(arg[0], arg[1], arg[2], arg[3]) == WRONG_PHONE_NUMBER_LENGTH) {
...
int AddAddressAt(char *arg[], struct Person *list)
{
	...
	if (is_valid_address(arg[0], arg[1], arg[2], arg[3]) == VALID) {
		...
		if (add_new_address_at_position(person, &list, (int)position) != 0) {
...
int ClearBook(struct Person **list)
{
	delete_all_addresses(list);
	printf("Deleted every address from the book\n");
}