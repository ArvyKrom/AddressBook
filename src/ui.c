...
int add_address_command(struct Person **list, char *arg[])
{
	...
	if (is_this_address_valid == WRONG_NAME_LENGTH) {
...
int del_address_command(struct Person **list, char *arg[])
{
	...
	if (delete_address(list, position) == 1) {
...
int main_loop(struct Person **list)
{
	...
	while (is_main_loop_running) {
		...
		if (ret == INVALID_ARGUMENT) {
			printf("\033[1m(!)\033[0m Seems like You have entered an argument that's wayyyyy too long\n");
			continue;
		}
